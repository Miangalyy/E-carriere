    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header" style="background: #2a2185; color: #fff;">
          <h5 class="modal-title" id="exampleModalLabel">Nouveau Direction</h5>
        ...
        <div class="modal-body">
          <form @submit.prevent="soumettre" id="createForm">
             <div class="form-group">
                <label for="nom">Numero_direction :</label>
             ...
             <div class="form-group">
                ...
                <input v-model="responsable" name="responsable" required type="text" class="form-control"/>
             </div>
             <div class="form-group">
...
const closeModal = ()=>{
  ...
  const soumettre = ()=>{
    const numero_direction = nd.value
    const res = responsable.value
    const title = titre.value
    const description = desc.value
    const url = route("direction.create")
    Inertia.post(url, {numero_direction, res, title, description}, {
      onSuccess: (page) => {
        closeModal()

        //Message de success
        useSwalSuccess("Direction ajouté avec success !")

      },
      onError: (Errors) => {
        if(Errors.numero_direction != null){
           ndError.value = Errors.numero_direction
           console.log(ndError)
