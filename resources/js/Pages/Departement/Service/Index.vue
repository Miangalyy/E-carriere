            ...
            <div class="card-wrapper swiper-wrapper">
                <div class="cardee swiper-slide" :key="test.id" v-for="test in props.Direction">
                    ...
                    <div class="card-content">
                        ...
                        <p class="description">Ce service possede {{ test.employer_count }} employés
                            </p>
                    </div>
                </div>                         
            </div>
            ...
            <div class="carde">
                <div class="cardHeader">
                    ...
                    <div class="card-tools">
                        ...
                        :prev="props.Services.prev_page_url"
                        :next="props.Services.next_page_url"
                        />
                    </div>
                </div>
                <div class="ext mt-4 mb-3">
                   <Link :href="route('service.create')" class="btn btn-primary boutton ml-4">
                ...
                <div class="card-body p-0">
                    <table class="table">
                        ...
                        <tbody>
                            <tr :key="Service.id" v-for="Service in props.Services.data">
                                <td>{{ Service.id }}</td>
                                <td>{{ Service.numero_service }}</td>
                                <td>{{ Service.responsable }}</td>
                                <td>{{ Service.direction.numero_direction }}</td>
                                <td>{{ Service.titre }}</td>
                                <td>{{ Service.description }}</td>
                                <td>
                                    <div class="d-flex justify-items-center">
                                        <Link :href="route('service.edit', {service: Service.id})" class="mr-2 status delivered"><i class="fas fa-pen"></i></Link>
                                        <button @click="deleteConfirmation(Service)" class="status pending"><i class="fas fa-trash"></i></button>
                                       </div>
                                </td>
                            </tr>
                        </tbody>
                        ...
                        <tr>
                            <td>
                                <div class="form-group">
                                    <label for="">Numero_Direction :</label>
                                    <select @change="recherche" v-model="filtreService" class="form-control">
                                        <option value=""></option>
                                        <option :key="Direction.id" :value="Direction.id" v-for="Direction in props.Directions">{{ Direction.numero_direction }}</option>
...
<script setup>
  ...
  const openEditModal = (id)=>{
   ...
   const deleteService = (id)=>{
    Inertia.delete(route("service.delete", {service: id}), {
      onSuccess: (response)=>{
   ...
   const deleteConfirmation = (Service)=>{
      const message = `Vous etes sur le point de supprimer le service numero : "${Service.numero_service}", voulez vous continuez ?`
  ...
  const recherche = _.throttle(function(){
    ...
    Inertia.get(route("service", {search: searchService.value, filter: filtreService.value, per_page: per_page.value}), {}, {
      replace: true,
      preserveState: true
    })
  })
