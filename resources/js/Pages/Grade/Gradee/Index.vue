        ...
        <div class="recentsOrder">
            <div class="carde">
                <div class="cardHeader">
                    <h2 class="title ml-4">Liste des grades</h2>
                ...
                <div class="card-body p-0">
                    <table class="table">
                        ...
                        <tbody>
                            <tr :key="Grade.id" v-for="Grade in props.Grades.data">
                                <td>{{ Grade.id }}</td>
                                <td>{{ Grade.numero_grade }}</td>
                                <td>{{ Grade.titre }}</td>
                                <td>{{ Grade.description }}</td>
                                <td>
                                    <div class="d-flex justify-items-center">
                                        <button @click="openEditModal(Grade.id)" class="mr-2 status delivered"><i class="fas fa-pen"></i></button>
                                        <button @click="deleteConfirmation(Grade.id)" class="status pending"><i class="fas fa-trash"></i></button>
                                       </div>
                                </td>
                            </tr>
                        </tbody>
...
<script setup>
  ...
  const openEditModal = (id)=>{
   ...
   const deleteDirection = (id)=>{
    Inertia.delete(route("grade.delete", {grade: id}), {
      onSuccess: (response)=>{
          useSwalSuccess("Grade supprimer avec success .")
          closeModal()
      },
      onError: (error)=>{
  ...
  const recherche = _.throttle(function(){
    ...
    Inertia.get(route("grade", {search: searchDirection.value, per_page: per_page.value}), {}, {
      replace: true,
      preserveState: true
    })
  })
