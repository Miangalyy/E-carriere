        ...
        <div class="modal-body">
          <form @submit.prevent="soumettre" id="createForm">
             <div class="form-group">
                <label for="nom">Numero_grade :</label>
...
const closeModal = ()=>{
  ...
  const soumettre = ()=>{
    const numero_grade = nd.value
    const title = titre.value
    const description = desc.value
    const url = route("grade.create")
    Inertia.post(url, {numero_grade, title, description}, {
      onSuccess: (page) => {
        closeModal()

        //Message de success
        useSwalSuccess("Grade ajouté avec success !")

      },
      onError: (Errors) => {
        if(Errors.numero_direction != null){
           ndError.value = Errors.numero_grade
           console.log(ndError)
