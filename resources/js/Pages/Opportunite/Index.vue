            ...
            <div class="card-wrapper swiper-wrapper">
                <div class="cardee swiper-slide" :key="test.id" v-for="test in props.Direction">
                    ...
                    <div class="card-content">
                        <h2 class="name">En {{ test.datee }}</h2>
                        <p class="description">{{ test.nombre }} employés sont inscrits du titre {{ test.titre }}
                            </p>
                    </div>
                </div>                         
            </div>
        ...
        <div class="recentsOrder">
            <div class="carde">
                <div class="cardHeader">
                    <h2 class="title ml-4">Liste des opportunités</h2>
                    <div class="card-tools">
                        <Pagination
                        :links="props.Opportunites.links"
                        :prev="props.Opportunites.prev_page_url"
                        :next="props.Opportunites.next_page_url"
                        />
                    </div>
                </div>
                ...
                <div class="card-body p-0">
                    <table class="table">
                        ...
                        <tbody>
                            <tr :key="Opportunite.id" v-for="Opportunite in props.Opportunites.data">
                                <td>{{ Opportunite.id }}</td>
                                <td>{{ Opportunite.titre }}</td>
                                <td>{{ Opportunite.description }}</td>
                                <td>{{ Opportunite.competence }}</td>
                                <td>{{ Opportunite.date }}</td>
                                <td>
                                    <div class="d-flex justify-items-center">
                                        <button @click="openEditModal(Opportunite.id)" class="mr-2 status delivered"><i class="fas fa-pen"></i></button>
                                        <button @click="deleteConfirmation(Opportunite.id)" class="status pending"><i class="fas fa-trash"></i></button>
                                       </div>
                                </td>
                            </tr>
                        </tbody>
...
const openEditModal = (id)=>{
 ...
 const deleteDirection = (id)=>{
  Inertia.delete(route("opportunite.delete", {opportunite: id}), {
    onSuccess: (response)=>{
        useSwalSuccess("Opportunité supprimer avec success .")
        closeModal()
    },
    onError: (error)=>{
