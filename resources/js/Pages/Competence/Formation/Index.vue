    ...
    <div class="detail">
        <div class="recentsOrder">
            <div class="carde">
                <div class="cardHeader">
                    <h2 class="title ml-4">Liste des formations</h2>
                    <div class="card-tools">
                        <Pagination
                        :links="props.Formations.links"
                        :prev="props.Formations.prev_page_url"
                        :next="props.Formations.next_page_url"
                        />
                    </div>
                </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Nom_Formation</th>
                                <th>Organisme_De_Formation</th>
                                <th>Description</th>
                                <th style="width: 30px;"></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr :key="Formation.id" v-for="Formation in props.Formations.data">
                                <td>{{ Formation.id }}</td>
                                <td>{{ Formation.nom_formation }}</td>
                                <td>{{ Formation.organisme_formation }}</td>
                                <td>{{ Formation.description }}</td>
                                <td>
                                    <div class="d-flex justify-items-center">
                                        <button @click="openEditModal(Formation.id)" class="mr-2 status delivered"><i class="fas fa-pen"></i></button>
                                        <button @click="deleteConfirmation(Formation.id)" class="status pending"><i class="fas fa-trash"></i></button>
                                       </div>
                                </td>
                            </tr>
                        </tbody>
        ...
        <div class="col-md-12 mt-2" style="background: #C18653; color: #fff; text-align: center;">
            <div style="color: #fff;padding: 18px; text-align: center; margin-right: 0px;font-weight: 400;">
                <Link :href="route('formemple')" class="ml-4" style="color: #fff; font-size: 25px;line-height: 10px; font-weight: 460;"><i class="fas fa-link mr-3"></i>Relier la formations á un employé</Link>
            </div>
        </div>
        <EditDirection
...
const openEditModal = (id)=>{
 ...
 const deleteDirection = (id)=>{
  Inertia.delete(route("formation.delete", {formation: id}), {
    onSuccess: (response)=>{
        useSwalSuccess("Formation supprimer avec success .")
        closeModal()
    },
    onError: (error)=>{
...
const recherche = _.throttle(function(){
  ...
  Inertia.get(route("formation", {search: searchDirection.value, per_page: per_page.value}), {}, {
    replace: true,
    preserveState: true
  })
})
