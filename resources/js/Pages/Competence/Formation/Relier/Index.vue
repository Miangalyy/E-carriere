            ...
            <div class="card-wrapper swiper-wrapper">
                <div class="cardee swiper-slide" :key="test.id" v-for="test in props.Evaluation">
                    ...
                    <div class="card-content">
                        <h2 class="name">En {{ test.date_debut }}</h2>
                        <p class="description">{{ test.nombre }} employés faisaient une formation
                            </p>
                    </div>
                </div>                         
            </div>
    ...
    <div class="detail">
        <div class="recentsOrder">
            <div class="carde">
                <div class="cardHeader">
                    <h2 class="title ml-4">Liste des formation</h2>
                    <div class="card-tools">
                        <Pagination
                        :links="props.relier.links"
                        :prev="props.relier.prev_page_url"
                        :next="props.relier.next_page_url"
                        />
                    </div>
                </div>
                <div class="ext mt-4 mb-3">
                   <Link :href="route('formemple.create')" class="btn btn-primary boutton ml-4">
                ...
                <div class="card-body p-0">
                    <table class="table">
                        ...
                        <tbody>
                            <tr :key="relie.id" v-for="relie in props.relier.data">
                                <td>{{ relie.numero }}</td>
                                <td>{{ relie.formation.nom_formation }}</td>
                                <td>{{ relie.employer.nom }} {{ relie.employer.prenom }}</td>
                                <td>{{ relie.date_debut }} - {{ relie.date_fin }}</td>
                                <td>
                                    <div class="d-flex justify-items-center">
                                        <Link :href="route('formemple.edit', {relier: relie.numero})" class="mr-2 status delivered"><i class="fas fa-pen"></i></Link>
                                        <button @click="deleteConfirmation(relie)" class="status pending"><i class="fas fa-trash"></i></button>
                                       </div>
                                </td>
                            </tr>
                        </tbody>
        ...
        <div class="col-md-12 mt-2" style="background: #C18653; color: #fff; text-align: center;">
            <div style="color: #fff;padding: 18px; text-align: center; margin-right: 0px;font-weight: 400;">
                <Link :href="route('formation')" class="ml-4" style="color: #fff; font-size: 25px;line-height: 10px; font-weight: 460;"><i class="fas fa-align-right mr-3"></i>Revenir á liste des formations</Link>
            </div>
        </div>   
    </div>
...
<script setup>
  ...
  const openEditModal = (id)=>{
   ...
   const deleteService = (id)=>{
    Inertia.delete(route("formemple.delete", {relier: id}), {
      onSuccess: (response)=>{
          useSwalSuccess("Formation supprimé avec success .")
          closeModal()
      },
      onError: (error)=>{
   ...
   const deleteConfirmation = (Service)=>{
      const message = `Vous etes sur le point de supprimer la formation numero : "${Service.numero}", voulez vous continuez ?`
  ...
  const recherche = _.throttle(function(){
    ...
    Inertia.get(route("formemple", {search: searchService.value, per_page: per_page.value}), {}, {
      replace: true,
      preserveState: true
    })
  })
