    ...
    <div class="detail">
            ...
            <div class="carde">
                ...
                <div class="card-body p-0">
                    <table class="table">
                        ...
                        <tbody>
                            <tr :key="Experience.id" v-for="Experience in props.Experiences.data">
                                <td>{{ Experience.id }}</td>
                                <td>{{ Experience.fonction }}</td>
                                <td>
                                    <div class="d-flex justify-items-center">
                                        <button @click="openEditModal(Experience.id)" class="mr-2 status delivered"><i class="fas fa-pen"></i></button>
                                        <button @click="deleteConfirmation(Experience.id)" class="status pending"><i class="fas fa-trash"></i></button>
                                       </div>
                                </td>
                            </tr>
                        </tbody>
        ...
        <div class="col-md-12 mt-2" style="background: #C18653; color: #fff; text-align: center;">
            <div style="color: #fff;padding: 18px; text-align: center; margin-right: 0px;font-weight: 400;">
                <Link :href="route('expemple')" class="ml-4" style="color: #fff; font-size: 25px;line-height: 10px; font-weight: 460;"><i class="fas fa-link mr-3"></i>Relier l'Expérience á un employé</Link>
            </div>
...
const openEditModal = (id)=>{
 ...
 const deleteDirection = (id)=>{
  Inertia.delete(route("experience.delete", {experience: id}), {
    onSuccess: (response)=>{
        useSwalSuccess("Expérience supprimer avec success .")
        closeModal()
    },
    onError: (error)=>{
...
const recherche = _.throttle(function(){
  ...
  Inertia.get(route("experience", {search: searchDirection.value, per_page: per_page.value}), {}, {
    replace: true,
    preserveState: true
  })
})
