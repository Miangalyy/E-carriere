<template>
        ...
        <div class="detail">
                ...
                <div class="carde">
                    <div class="cardHeader">
                        ...
                        <div class="card-tools">
                            ...
                            :prev="props.Evaluations.prev_page_url"
                            :next="props.Evaluations.next_page_url"
                            />
                        </div>
                    </div>
                    ...
                    <div class="card-body p-0">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Titre de l'evaluation</th>
                                    <th style="width: 100px;"></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr :key="Evaluation.id" v-for="Evaluation in props.Evaluations.data">
                                    <td>{{ Evaluation.id }}</td>
                                    <td>{{ Evaluation.titre }}</td>
                                    <td>
                                        <div class="d-flex justify-items-center">
                                            <button @click="openEditModal(Evaluation.id)" class="mr-2 status delivered"><i class="fas fa-pen"></i></button>
                                            <button @click="deleteConfirmation(Evaluation.id)" class="status pending"><i class="fas fa-trash"></i></button>
                                           </div>
                                    </td>
                                </tr>
                            </tbody>
            ...
            <div class="col-md-12 mt-2" style="background: #C18653; color: #fff; text-align: center;">
                <div style="color: #fff;padding: 18px; text-align: center; margin-right: 0px;font-weight: 400;">
                    <Link :href="route('evalemple')" class="ml-4" style="color: #fff; font-size: 25px;line-height: 10px; font-weight: 460;"><i class="fas fa-link mr-3"></i>Relier l'Evaluation á un employé</Link>
                </div>
    ...
    const openEditModal = (id)=>{
     ...
     const deleteDirection = (id)=>{
      Inertia.delete(route("evaluation.delete", {evaluation: id}), {
        onSuccess: (response)=>{
            useSwalSuccess("Evaluation supprimer avec success .")
            closeModal()
        },
        onError: (error)=>{
    ...
    const recherche = _.throttle(function(){
      ...
      Inertia.get(route("evaluation", {search: searchDirection.value, per_page: per_page.value}), {}, {
        replace: true,
        preserveState: true
      })
    })
