            ...
            <div class="card-wrapper swiper-wrapper">
                <div class="cardee swiper-slide" :key="test.id" v-for="test in props.Evaluation">
                    ...
                    <div class="card-content">
                        ...
                        <p class="description">{{ test.nombre }} employés sont évalués
                            </p>
                    </div>
                </div>                         
            </div>
    ...
    <div class="detail">
        <div class="recentsOrder">
            <div class="carde">
                <div class="cardHeader">
                    <h2 class="title ml-4">Liste des evaluations</h2>
                ...
                <div class="ext mt-4 mb-3">
                   <Link :href="route('evalemple.create')" class="btn btn-primary boutton ml-4">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr :key="relie.id" v-for="relie in props.relier.data">
                                <td>{{ relie.numero }}</td>
                                <td>{{ relie.evaluation.titre }}</td>
                                <td>{{ relie.commentaire }}</td>
                                <td>{{ relie.employer.nom }} {{ relie.employer.prenom }}</td>
                                <td>{{ relie.date }}</td>
                                <td>
                                    <div class="d-flex justify-items-center">
                                        <Link :href="route('evalemple.edit', {relier: relie.numero})" class="mr-2 status delivered"><i class="fas fa-pen"></i></Link>
        ...
        <div class="col-md-12 mt-2" style="background: #C18653; color: #fff; text-align: center;">
            <div style="color: #fff;padding: 18px; text-align: center; margin-right: 0px;font-weight: 400;">
                <Link :href="route('evaluation')" class="ml-4" style="color: #fff; font-size: 25px;line-height: 10px; font-weight: 460;"><i class="fas fa-align-right mr-3"></i>Revenir á liste des evaluations</Link>
            </div>
        </div>   
    </div>
...
<script setup>
  ...
  const openEditModal = (id)=>{
   ...
   const deleteService = (id)=>{
    Inertia.delete(route("evalemple.delete", {relier: id}), {
      onSuccess: (response)=>{
          useSwalSuccess("Evaluation supprimé avec success .")
          closeModal()
      },
      onError: (error)=>{
   ...
   const deleteConfirmation = (Service)=>{
      const message = `Vous etes sur le point de supprimer l'evaluation numero : "${Service.numero}", voulez vous continuez ?`
  ...
  const recherche = _.throttle(function(){
    ...
    Inertia.get(route("evalemple", {search: searchService.value, per_page: per_page.value}), {}, {
      replace: true,
      preserveState: true
    })
  })
