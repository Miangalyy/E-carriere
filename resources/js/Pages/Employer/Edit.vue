<template>
      ...
      <section class="content">
        ...
        <div class="row">
          ...
          <div class="col-md-10">
            ...
            <h3 class="card-title">Édition d'un employé</h3>
            </div>
                                        ...
                                        <div class="col-6">
                                          <label for="exampleInputFile">Photo :</label>
                                            <div class="custom-file">
                                              <input :key="inputKey" type="file" class="custom-file-input" id="exampleInputFile" @input="previewtImage($event)">
                                              <label class="custom-file-label" for="exampleInputFile">Choose file</label>
                                            </div>
                                            
                                        </div>
                                        <div v-if="employer.photo" class="col-2" style="margin-left: 80px; margin-top: 10px;">
                                            <img :src="'/storage/'+employer.photo" id="image-preview" style="width:75px;height:75px;border-radius: 25px;">
                                        </div> 
                                        <div v-else class="col-2" style="margin-left: 80px; margin-top: 10px;">
    ...
    <script setup>
       ...
       const form = useForm({
        numero_employer: props.employer.numero_employer,
        nom: props.employer.nom,
        prenom: props.employer.prenom,
        adresse: props.employer.adresse,
        date_naissance: props.employer.date_naissance,
        date_embauche: props.employer.date_embauche,
        email: props.employer.email,
        telephone: props.employer.telephone,
        sexe: props.employer.sexe,
        grade_id: props.employer.grade_id,
        corps_id: props.employer.corps_id,
        fonction_id: props.employer.fonction_id,
        photo: null
        
       })
       const soumettre = ()=>{
        form.post(route("employer.update", {employer: props.employer.id}), {
            onSuccess: (page)=>{
                useSwalSuccess("Employé mis á jours avec success !")
                form.reset()
                inputKey.value += 1
                resetForm();
